...
--
-- Tenant seed: Ford — starter plan, tenant_admin role.
-- ============================================================================
--
...
--   - Creates or updates the public.tenants row for Ford
...
DECLARE
  ...
  c_seed_email              TEXT    := 'admin@ford.com';         -- ← change to actual Ford admin email
  c_tenant_display_name     TEXT    := 'Ford';
  c_full_name               TEXT    := 'Ford Admin';             -- ← change to actual name
  c_plan                    TEXT    := 'starter';                -- starter | professional | enterprise
  c_workflow_addon_slots    INTEGER := 0;                        -- 0–5
  c_agent_packs             TEXT[]  := ARRAY['underwriting'];    -- starter gets 1 pack
  c_device_name             TEXT    := 'Ford Admin primary device';
  c_signup_wizard_version   TEXT    := 'v1';
  c_device_profile          JSONB   := jsonb_build_object(
    'device_name',                'Ford Admin primary device',
    'device_type',                'desktop',
    'os_name',                    'Windows',
    'os_version',                 '11',
