...
--
-- Bootstrap seed: create the first global-admin user and tenant.
-- ============================================================================
--
...
-- 2) Edit the CONFIG block below (email, full_name, tenant name, plan, packs).
...
--   - Creates or updates the public.tenants row for this admin
...
DECLARE
  ...
  c_seed_email              TEXT    := 'samarthbhujadi@gmail.com';
  c_tenant_display_name     TEXT    := 'BMW';
  c_full_name               TEXT    := 'samarth';
  c_plan                    TEXT    := 'professional'; -- starter | professional | enterprise
  c_workflow_addon_slots    INTEGER := 0;              -- 0–5
  c_agent_packs             TEXT[]  := ARRAY['underwriting', 'claims', 'distribution'];
  c_device_name             TEXT    := 'Admin primary device';
  c_signup_wizard_version   TEXT    := 'v1';
  c_device_profile          JSONB   := jsonb_build_object(
    'device_name',                'Admin primary device',
    'device_type',                'desktop',
    'os_name',                    'Windows',
    'os_version',                 '11',
