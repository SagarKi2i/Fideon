...
import {
  Loader2, ShieldAlert, Eye, RefreshCw, CheckCircle2, XCircle,
  Filter, SortAsc, SortDesc, ChevronLeft, ChevronRight,
} from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import {
  listAcordAdminQueue, adminQueueStats, batchReviewAcordRuns,
  getRunHealthCard,
...
const FORM_TYPES = ["25", "27", "80", "85", "90", "125", "126", "140"];
const PAGE_SIZE = 25;

export default function AdminAcordQueue() {
  ...
  const [orderBy, setOrderBy] = useState<"priority" | "created_at">("priority");
  ...
  const loadStats = async () => {
    ...
    try {
      const data = await adminQueueStats();
      setStats(data);
    } catch {
      // Non-critical
    } finally {
  ...
  const buildFilters = useCallback((p: number): AdminQueueFilters => {
    const f: AdminQueueFilters = {
    ...
    if (filterFormType) f.form_type = filterFormType;
    if (filterConfMin !== "") f.conf_min = Number(filterConfMin) / 100;
    if (filterConfMax !== "") f.conf_max = Number(filterConfMax) / 100;
    return f;
  }, [filterStates, filterFormType, filterConfMin, filterConfMax, orderBy, orderDir]);

  const load = useCallback(async (p = 1) => {
    setLoading(true);
    setSelected(new Set());
    try {
      const data = await listAcordAdminQueue(buildFilters(p));
      setItems(data.queue);
      setPage(p);
  ...
  useEffect(() => {
    ...
    Promise.all(
      runIds.map(async runId => {
        try {
          const [card, historyResp] = await Promise.all([
            getRunHealthCard(runId),
            getJobHistoryByRunId(runId, 25),
          ]);
          return [runId, card, historyResp?.jobs || []] as const;
        } catch {
      ...
      const nextHistory: Record<string, any[]> = {};
      for (const entry of entries) {
        const runId = String(entry?.[0] || "");
        const card = entry?.[1];
        const jobs = Array.isArray(entry?.[2]) ? entry[2] : [];
        if (card) next[runId] = card;
        if (runId) nextHistory[runId] = jobs;
      }
      setHealthByRunId(next);
      setHistoryByRunId(nextHistory);
    });
    return () => {
      cancelled = true;
    };
  ...
  const handleBatch = async (decision: "approve" | "reject") => {
    ...
    try {
      const result = await batchReviewAcordRuns([...selected], decision);
      toast({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Training Admin Review Queue</h1>
      ...
      <Card>
          ...
          <CardContent className="border-b border-border/50 pb-4 pt-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Select
                  value={filterStates}
                  onValueChange={v => setFilterStates(v)}
                >
              ...
              <div className="space-y-1">
                ...
                <Select value={filterFormType || "__all"} onValueChange={v => setFilterFormType(v === "__all" ? "" : v)}>
                  <SelectTrigger className="h-8 text-xs">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-1.5">
                  <Select value={orderBy} onValueChange={v => setOrderBy(v as "priority" | "created_at")}>
        ...
        <CardContent className={`space-y-3 ${showFilters ? "pt-4" : ""}`}>
          ...
          {visibleItems.map((row: any) => {
            ...
            const health = healthByRunId[runId];
            const calibratedPct = Math.round(Number(health?.confidence_evaluation?.calibrated_confidence ?? conf) * 100);
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <Checkbox
                    checked={isChecked}
                    onCheckedChange={() => toggleOne(runId)}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant={stateMeta.variant} className="text-[10px] h-4">{stateMeta.label}</Badge>
                      {run.form_type_detected && (
                        <Badge variant="outline" className="text-[10px] h-4 text-primary border-primary/30">
                          ACORD {run.form_type_detected}
                        </Badge>
                      )}
                      <Badge
                    ...
                    {/* Fine-tuning history (latest first) */}
                    {historyByRunId[runId]?.length ? (
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {historyByRunId[runId].map((j: any) => {
                          ...
                          return (
                            <Badge
                              key={String(j?.id || `${runId}-${st}-${j?.created_at || ""}`)}
                  ...
                  <Button
                    size="sm"
                    onClick={() => navigate(`/admin/acord-queue/${runId}`)}
