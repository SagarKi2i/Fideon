...
def _build_comparison_prompt(
    ...
    return (
        ...
        "- Always include extracted_fields.policyA and extracted_fields.policyB with key fields (carrier, premiums, limits, deductibles, effective dates, exclusions).\n"
...
@router.post("/compare")
async def compare_acord_policies(
    ...
    """
    Extract both policy documents with the full ACORD pipeline (OCR + LLM) and
    compare them. Returns a PolicyComparisonStructured JSON object suitable for
    the Policy Comparison Engine UI.
    """
    ...
    logger.info(
        "ACORD[compare] request: file_a=%s file_b=%s threshold=%s user_id=%s",
        file_a.filename, file_b.filename, deviation_threshold_percent, user_id,
    )
    ...
    try:
        ...
        logger.exception("ACORD[compare] extraction failed: %s", exc)
    ...
    prompt = _build_comparison_prompt(summary_a, summary_b, name_a, name_b, deviation_threshold_percent)

    try:
        result = await _compare_via_llm(prompt)
    except Exception as exc:
        logger.exception("ACORD[compare] LLM call failed: %s", exc)
        raise HTTPException(status_code=502, detail=f"LLM comparison failed: {exc}") from exc

    logger.info("ACORD[compare] done: file_a=%s file_b=%s user_id=%s", name_a, name_b, user_id)
    return result
