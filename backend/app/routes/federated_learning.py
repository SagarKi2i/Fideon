...
async def federated_learning(request: Request, x_device_token: Optional[str] = Header(default=None)):
    ...
    if action == "get-feedback":
        query = f"select=*&device_id=eq.{quote(device['id'], safe='')}&order=created_at.desc"
        if model_id:
            query += f"&model_id=eq.{quote(model_id, safe='')}"
        if request.query_params.get("unused") == "true":
    ...
    if action == "get-training-jobs":
        rows = await postgrest_get(
            "training_jobs",
            f"select=*&device_id=eq.{quote(device['id'], safe='')}&order=created_at.desc",
        )
    ...
    if action == "get-active-rounds":
        ...
        contributions = await postgrest_get(
            "federated_updates",
            f"select=model_id,round_number,status,submitted_at&device_id=eq.{quote(device['id'], safe='')}",
        )
        return {"success": True, "rounds": rounds, "contributions": contributions}

    if action == "get-stats":
        feedback = await postgrest_get("training_feedback", f"select=id&device_id=eq.{quote(device['id'], safe='')}")
        jobs = await postgrest_get("training_jobs", f"select=id&device_id=eq.{quote(device['id'], safe='')}")
        contribs = await postgrest_get("federated_updates", f"select=id&device_id=eq.{quote(device['id'], safe='')}")
        return {
            "success": True,
            "stats": {
    ...
    if action == "create-training-job":
        feedback = await postgrest_get(
            "training_feedback",
            (
                f"select=id&device_id=eq.{quote(device['id'], safe='')}"
        ...
        await postgrest_patch(
            "training_feedback",
            (
                f"device_id=eq.{quote(device['id'], safe='')}"
                f"&model_id=eq.{quote(str(body.get('model_id')), safe='')}"
                "&is_used_for_training=eq.false"
            ),
    ...
    if action == "update-training-job":
        ...
        await postgrest_patch(
            "training_jobs",
            f"id=eq.{quote(str(body.get('job_id')), safe='')}&device_id=eq.{quote(device['id'], safe='')}",
            update_data,
        )
        return {"success": True}
